...
## General hitting-time / upcrossing utility (cross-cutting)

This generalizes the first-passage/ruin-time utility flagged in Section 5 into something that applies to essentially every process on this list, not just the actuarial surplus process. The key architectural fact: RV.apply(func) already returns RV(prob_space, lambda outcome: func(self.func(outcome))), and for a RandomProcess, self.func(outcome) is the entire realized path object, not a single value. So a plain Python function that takes a whole path and returns a hitting time already works today via the existing apply() mechanism with no new plumbing -- the real work is writing hitting_time and upcrossings themselves, and their difficulty splits into three tiers depending on what kind of path the process produces, rather than being one uniform feature.

| Process | Priority | Why it matters | Difficulty & implementation notes |
|---|---|---|---|
| Tier A -- discrete-time or pure-jump processes (Random Walk, MarkovChain, AR/MA, Renewal, Poisson/Compound Poisson, birth-death queues, CTMC) | High | Covers most processes on this list with no new algorithmic risk. | Easy, and exact. These processes are either integer-indexed, or are step functions fully determined by a lazily-generated, exactly-known sequence of jump times. A generic hitting_time just walks path[0], path[1], ... (or the jump-time sequence) until it finds a crossing, with a max_time/max_steps cutoff to avoid looping forever if the level is never hit. Nothing is hidden between queried points, so no approximation is involved. |
| Tier B -- Brownian motion / Gaussian processes | High | BM and OU are among the most central processes on the whole list, so this tier matters even though it needs real work. | Moderate, but exact. GaussianProcessResult only knows values at times actually queried, so a level crossing could hide between two sampled points. The fix is a known, well-trodden technique: the probability that a Brownian bridge between two known non-crossing points crosses the level has a closed form (reflection principle); draw a Bernoulli using that probability to decide whether a crossing happened in the gap, then bisect to localize the exact time -- the same bisection machinery already built for DiffusionProcess's zoom-in behavior, just answering a crossing question instead of a value question. |
| Tier C -- general diffusions (DiffusionProcess, CIR, CEV, CKLS, etc.) | Medium | Extends the utility to the SDE family, but inherits that family's own caveats. | Moderate, but approximate. No closed-form bridge-crossing probability exists for a general SDE, so this reuses the same Euler-Maruyama plus mesh-refinement approach already flagged as approximate for DiffusionProcess, with accuracy controlled by the same tol parameter. Same honest caveat as the rest of DiffusionProcess: consistent and refinable, not exact. |
| Upcrossings (as a sequence, once hitting_time exists for a process) | Medium-High | The natural generalization once single-hit detection works; requested explicitly alongside hitting times. | Nearly free once hitting_time works for a process: find the first crossing, then look for the next one after that, and repeat. The natural representation is an InfiniteVector -- lazily generating the nth upcrossing time on demand -- which is exactly the caching pattern already used throughout the codebase (MarkovChainResult, GaussianProcessResult), so this is a thin loop around Tier A/B/C rather than a fourth thing to build. |

Recommendation: build this as one shared utility with per-process-type dispatch (Tier A generically for any discrete-time or jump-type Result, Tier B specifically for Gaussian-process-family Results, Tier C for DiffusionProcess), rather than as three unrelated one-off features. It also supersedes the standalone first-passage/ruin-time utility row in Section 5 -- that entry becomes the actuarial use case of this general mechanism rather than a separate build.
...
Revised to weave in the general hitting-time/upcrossing utility alongside the survival/point-process, Bayesian nonparametric, queueing, and cross-discipline additions:
...
- 7. Hitting-time / upcrossing utility, Tier A (discrete-time and pure-jump processes) -- easy and exact, and immediately useful across nearly everything built in steps 1-6; do this early since it multiplies the value of each process already in place.
- 8. Polya urn scheme and Chinese Restaurant Process -- cheap, high pedagogical value, reuse the existing recursive-caching pattern; do these before attempting the Dirichlet process itself.
- 9. Merge / finish DiffusionProcess -- document the approximation caveat clearly; then CIR and other SDEs (CEV, CKLS) become near-free parameterizations.
- 10. Hitting-time / upcrossing utility, Tier B (Brownian motion / Gaussian processes) -- moderate but exact via the reflection-principle bridge-crossing trick; high value given how central BM/OU are.
- 11. MA(q), then AR(p)/ARMA(p,q) -- MA is easy standalone; AR needs the recursive-caching design decision, ARMA combines both.
- 12. G/G/1 queue via Lindley's recursion (built on RenewalProcess for arrivals) -- M/G/1 and G/M/1 fall out for free as special cases.
- 13. Non-homogeneous Poisson process -- needed both for the Weibull/power-law reliability framing and as the direct prerequisite for the Cox process and Hawkes process below.
- 14. Cox process (built on non-homogeneous Poisson thinning) -- also subsumes the Markov-modulated Poisson process from the actuarial section as a special case, so build once rather than twice.
- 15. Mixed Poisson process, GARCH/ARCH, Galton-Watson, Hidden Markov Model -- all moderate, all benefit from the machinery built in steps 1-14.
- 16. Fractional Brownian Motion -- cheap to add, note the covariance-matrix performance caveat in docs.
- 17. Hawkes process and shot-noise/filtered Poisson process -- build once Compound Poisson and non-homogeneous Poisson exist.
- 18. Hitting-time / upcrossing utility, Tier C (general diffusions) -- moderate, approximate, inherits DiffusionProcess's own caveats; lower urgency than Tiers A/B.
- 19. Dirichlet process -- resolve the architectural question (own object vs. RandomProcess subclass) before implementing; genuinely new design work, not just new code.
- 20. Multi-state survival/actuarial CTMC worked examples, continuous-time N(t) for G/G/1, Merton jump-diffusion -- documentation-and-examples gaps or moderate new work; lower urgency.
- 21. G/G/s multi-server queues, queueing networks (Jackson networks), Pitman-Yor/Indian Buffet/Beta process -- lower-priority grad-level topics with narrower audiences; revisit once the rest of the roadmap is in place.