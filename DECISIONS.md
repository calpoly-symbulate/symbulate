...
**Status:** Finalized — implemented in `plot.py` (message constants) and `results.py` (dispatch + guards).

**Decision**
> Overlay whenever naturally possible. A second `.plot()` call always attempts to draw on the same axes. No automatic side-by-side behavior. Three outcomes: (1) **Natural overlay** — draws on shared axes, no message (scatter, impulse, 1D histogram, 1D density, rug, 2D density/contour, ecdf, most types); (2) **Readability warning** — overlay produced, message printed; (3) **Hard error** — the plots cannot be joined *or cannot be read correctly under any labeling*, `ValueError` raised. Color cycling via `get_next_color(ax)`, called once per `.plot()` call. Side-by-side panels are deferred.
>
> **The line between tiers is severity, not mechanism.** The tier definition above used to read "layout cannot be joined," which described how the first hard-error case happened to fail rather than what the tier is for. A warned overlay is *hard to read but honest* — the reader can see the problem and work around it. A hard error is for an overlay the reader cannot detect is wrong: either the figure is visibly corrupt (a shared multi-panel layout) or it looks perfect and misdescribes itself (a pdf/pmf drawn with a cdf, below). The second kind is the more dangerous of the two in a teaching context, which is why the definition is stated in terms of the outcome.
>
> **Warning cases (finalized wording as named constants in `plot.py`):** two tile plots (`TILE_OVERLAY_WARNING`), two 2D histograms (`HIST2D_OVERLAY_WARNING`), two mosaics (`MOSAIC_OVERLAY_WARNING`), two violins (`VIOLIN_OVERLAY_WARNING`), and a second 2D density (competing color scales or overlapping shapes). This is broader than the original "two tiles / two 2D histograms" list — every competing-colormap or overlapping-shape type gets the same treatment, on the same rationale. All follow the "what happened + why + what to do (separate cells, or `type='...'`)" template and are `print`ed (not `warnings.warn`), matching the suggestion-message system so they always show in Jupyter regardless of warning filters.
>
> **Hard-error case (finalized):** any overlay involving a `marginal=True` layout — a second `.plot()` on a figure that already carries a marginal layout, or a `marginal=True` request on a figure that already has a plot. The three-panel GridSpec can't be shared; a second plot would draw into one of the marginal strips at mismatched coordinates. `RVResults.plot()` raises `MARGINAL_OVERLAY_ERROR` (a student-friendly `ValueError`) instead. The figure is tagged `_symbulate_marginal` when the layout is built; a fresh Jupyter cell gets a fresh figure, so the guard only fires on a genuine second plot.
>
> **Hard-error case (finalized):** a theoretical pdf/pmf and a theoretical cdf on the same axes — `Normal(0, 1).plot(); Normal(0, 1).plot(cdf=True)`. `Distribution.plot()` tags the *axes* with `_symbulate_theoretical_kind` (`"pdf/pmf"` or `"cdf"`) and raises `THEORETICAL_CDF_PDF_OVERLAY_ERROR` when the next call's kind differs and the axes already has data. The check runs before anything is drawn, so a refused overlay leaves the first plot intact. **This is reversible to a readability warning** — see "Reverting this to a warning" below.

**Rationale**
> Overlay is the expected behavior in a Jupyter teaching context. The three-tier policy avoids silently producing unreadable plots while still defaulting to overlay. **Divergence from the reference fork is intentional:** `team/phase1_symbulate_plot_comparison.ipynb` found the fork silently produces two overlaid tiles (no warning) and silently draws a second plot after `['scatter', 'marginal']` (no error). Both are the failure the policy exists to prevent — an unreadable plot and a corrupt figure — so the redesign fixes them rather than matching the fork. Side-by-side panels require a separate design decision (likely `panel=True` or a `.compare()` method) and are deferred so the `SymbulatePlot` wrapper can be designed to support it later.

**Rationale — why pdf/cdf is a hard error and not a warning**
> Bypassing the guard to see what the overlay actually produces: `ylim (0.0, 1.05)`, `ylabel 'Density'`, `title 'Probability Density Function'` — with a cumulative distribution curve drawn on it. Both curves render cleanly and the plot then asserts that a cdf is a density. Four reasons that lands in the hard-error tier:
>
> 1. **The two options are not equally reversible.** An error can be downgraded to a warning later without breaking anything that worked; a warning cannot be upgraded to an error without breaking notebooks that were "fine." Most of the visual design here is explicitly provisional, so the team will revisit this — starting strict keeps that door open at no cost.
> 2. **The failure modes are lopsided in a teaching context.** A wrongly-blocked student reads a message that names the fix and recovers in seconds. A student who misses a printed warning — easy in Jupyter, where the eye goes to the figure and not the stdout above it — keeps a plot that lies, possibly into a submission. One is self-correcting; the other is silent wrong learning.
> 3. **It is worse than the existing hard-error case in the way that matters.** A `marginal=True` collision produces a *visibly* corrupt figure. This produces a figure that looks perfect and misdescribes itself, which is the one failure a student cannot detect unaided.
> 4. **The blast radius is exactly the incoherent combination.** A simulated plot never sets the tag, so the two overlays that carry real teaching value are untouched: an empirical ECDF with a theoretical cdf drawn over it, and a histogram with a theoretical pdf over it. Two pdfs, two cdfs, and a pmf with a pdf all still overlay freely. Only pdf-with-cdf is refused.

**Reverting this to a warning**
> Recorded deliberately, because the reversibility argument above is only worth anything if the path back is written down. What would justify it: a lab, lecture, or notebook in the course that deliberately puts a pdf and a cdf on one set of axes. Nobody could name one when this was decided, but the team has visibility the implementer did not.
>
> The revert is: change the `raise` in `Distribution.plot()` to a `print` (**not** `warnings.warn` — see the warning-case note above for why printed messages are the convention); rename `THEORETICAL_CDF_PDF_OVERLAY_ERROR` to `..._WARNING` and reword its tail; move this case up to the warning list; and invert the `assertRaises` tests in `TestTheoreticalCDFPDFOverlayGuard`. **One extra step that is easy to miss:** `Distribution.plot()` now keeps the *first* title rather than overwriting (see the keep-first change landed alongside this guard), so a warned overlay would sit permanently titled "Probability Density Function" while displaying a cdf as well. A warning tier therefore also needs a neutral title for the mixed case — the revert is not a one-line change.

**Alternatives Considered**
> Error on any second call — too restrictive. Automatic side-by-side — requires multi-panel layout incompatible with the overlay architecture. Matching the reference fork's silent behavior (no tile warning, no marginal error) — rejected; that silence is the bug the policy exists to fix.
>
> For the pdf/cdf case specifically: a **readability warning** (rejected on the four points above, but explicitly reversible — see "Reverting this to a warning"); a **warning plus a neutral title** for the mixed axes (the honest form of the warning option, and the shape a revert should take, but it still leaves an unreadable double y-scale behind); and **auto-splitting into two panels**, which is the eventual right answer and is already deferred as its own decision (`panel=True` / `.compare()`). When that lands, `THEORETICAL_CDF_PDF_OVERLAY_ERROR`'s message is the natural place to point at it — a further reason an error beats a warning here, since it is already the hook.
...
>
> **Resolved (signature):** `curve(distribution)` accepts a **Symbulate distribution object** (e.g. `curve(Normal(0, 1))`), not a raw pdf/pmf function. It fits the rest of the package's vocabulary, and a distribution knows whether it is discrete (pmf → dots + dashed connector, per the `Distribution.plot()` discrete-rendering decision) or continuous (smooth curve), so `curve` can reuse the existing `Distribution.plot()` render path; a bare function carries no discreteness information and would misrender a pmf as a continuous curve. A friendly `TypeError` should reject non-distribution arguments when this is built. Implementation stays deferred with the rest of the Composition API.
>
...
- [x] Overlay warning text: finalized. The student-friendly wording lives as named constants in `plot.py` (`TILE_OVERLAY_WARNING`, `HIST2D_OVERLAY_WARNING`, `MOSAIC_OVERLAY_WARNING`, `VIOLIN_OVERLAY_WARNING`, and the 2D-density readability warning), all following the "what happened + why + what to do (separate cells, or `type='...'`)" template and `print`ed (not `warnings.warn`) so they always show in Jupyter regardless of warning filters — see "Decision: Overlay Behavior"
- [x] Overlay hard-error text: finalized and implemented. `MARGINAL_OVERLAY_ERROR` in `plot.py`; a second `.plot()` on a figure that already carries a `marginal=True` layout — and the reverse, a `marginal=True` request on a figure that already has a plot — raises a student-friendly `ValueError` from `RVResults.plot()` rather than silently drawing into a marginal strip. `THEORETICAL_CDF_PDF_OVERLAY_ERROR` joins it for a theoretical pdf/pmf overlaid with a cdf, tagged per-axes rather than per-figure. See "Decision: Overlay Behavior"
- [x] Theoretical pdf/cdf overlay tier: resolved as a **hard error**, deliberately kept reversible to a readability warning — the rationale and the exact revert steps are recorded under "Decision: Overlay Behavior". The same decision restated the hard-error tier itself in terms of severity ("cannot be read correctly under any labeling") rather than mechanism ("layout cannot be joined"), since this case is the one that separates the two
- [x] Overlay behavior discrepancy: resolved in favor of our reimplementation, not the fork. The fork's silent non-warning on two overlaid tiles and its non-error after `['scatter', 'marginal']` are the bugs the redesign fixes (both silently produce unreadable/corrupt figures), not behavior to match. We warn on two tiles / 2D histograms / mosaics / violins / 2D densities and hard-error on any overlay involving a `marginal=True` layout — see "Decision: Overlay Behavior"
...
- [x] `curve(distribution)`: resolved — accepts a **Symbulate distribution object** (e.g. `curve(Normal(0, 1))`), not a raw pdf/pmf function. A distribution knows whether it is discrete (pmf → dots + dashed connector, per finding #16) or continuous (smooth curve) and can reuse the existing `Distribution.plot()` render path; a bare function carries no discreteness info and would misrender a pmf as a continuous curve. Implementation stays deferred with the rest of the Composition API. See "Decision: Plot Composition API"
...
- [x] Violin plot overlays at large `k`: resolved (decision only, no code change) — violins are **not binned**, and ridgeline does **not** replace violin. A discrete axis with more than `K_2D` distinct values is already reclassified as continuous and routed to a 2D histogram, so an auto-selected violin has `k ≤ K_2D` by construction; binning a discrete axis would misrepresent it (a violin over "counts 5–9" implies those integers are one category). Violin stays the small-`k` comparison plot; **ridgeline (the filled segmented density, reachable via `type="segmented_density"` with `ridge=True`) is the recommended large-`k` alternative**. A first-class `type="ridgeline"` alias and a crowding hint were considered but deliberately not added, to avoid a second spelling for `ridge=True`. See "Decision: Default Plot Lookup Table"
