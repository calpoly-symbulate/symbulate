...
**Ordering note, since resolved:** the roadmap's build order puts **Tier A**
(discrete-time and pure-jump processes) at step 7 and Tier B at step 10, on the
grounds that Tier A is both easier and *exact* — it just walks `path[0],
path[1], ...` or the jump-time sequence with a cutoff, since nothing is hidden
between queried points. PR #272 built Tier B first and left Tier A unbuilt, so
for a while the cheap-and-exact half of the utility was missing while the subtle
half existed, and `RandomWalk`, `MarkovChain`, `RenewalProcess`, the birth-death
queues, and the `GG1`/`GGs` queues — most of the processes in the package —
could not answer a hitting-time question at all. **Tier A has since been added
to the same function**; see "Hitting Times — Tier A, in the Same Function as
Tier B" below, which also records the `isinstance(process, RandomProcess)` bug
this ordering left behind (most Tier A processes are plain `RV`s, so the
process-vs-path dispatch silently failed for them). Note one claim in the
sentence above did *not* survive contact: `NonHomogeneousPoissonProcess` and
`CoxProcess` counts jump but are still not covered, since their jump times are
known on the expected-count scale rather than the clock.
...
already documents. Its test file reseeds `gaussian_process.rng` and
`hitting_times.rng`, and, since Tier A landed, `distributions.rng` and
`markov_chains.rng` as well — a jump or discrete-time path is generated by
whichever module owns the process, not by this one.

---

## Decision: Hitting Times — Tier A, in the Same Function as Tier B

**Status:** Implemented — the jump and discrete-time branches of
`hitting_time` live in `symbulate/hitting_times.py`, tested in
`symbulate/tests/test_hitting_times.py`, demo in
`team/models-and-sim-design/hitting_times_demo.ipynb`. Nothing new is
exported; `hitting_time` was already public. This is roadmap step 7 (the
roadmap's own suggested order puts Tier A *before* Tier B, which shipped
first — no consequence, since the tiers share only the entry point).

**Decision**
> Tier A is **not a second function**. `hitting_time` keeps its one signature
> and dispatches on the *drawn path*, in this order:
> 1. **A pure-jump path** — `DiscreteValued` whose `get_states()` and
>    `get_interarrival_times()` both work. Walked jump by jump, accumulating
>    holding times, reporting the clock time of the jump that first reached the
>    level. Covers `PoissonProcess`, `RenewalProcess`,
>    `CompoundPoissonProcess`, `ContinuousTimeMarkovChain`, `BirthDeathProcess`
>    and the `MM*` queues, `GG1`/`GGs`.
> 2. **A discrete-time path** — `InfiniteTuple` or `DiscreteTimeFunction`.
>    Walked step by step. Covers `RandomWalk`, `MarkovChain`, `MA`.
> 3. **A Gaussian path** — unchanged Tier B machinery.
>
> Both new branches are **exact**, and `step` and `tol` are **ignored** by
> them. Those two arguments exist to deal with what a *continuous* path does
> between two evaluated times; a jump path holds one value at a time so it can
> only reach a level at a jump, and a discrete-time path has no values between
> its steps at all. There is nothing to scan past and nothing to localize.

**Three semantic choices that are not shared with Tier B**
> - **Reaching a level means reaching it *or passing it*.** A walk going from 4
>   to 6 is never at 5, and a count going from 3 to 4 is never at 3.5, so
>   equality would report `inf` for crossings that plainly happened. This is
>   the standard definition for these processes and it is decided in exactly one
>   place, `_reached`. For a Gaussian path the question does not arise, since a
>   continuous path cannot get to the other side without touching it.
> - **A discrete-time answer is a time, and `max_time` counts steps.** For a
>   random walk, a Markov chain, or an `MA` series a step *is* the unit of time
>   — the index set is the clock — so no second `max_steps` argument was added
>   (the roadmap row says "max_time/max_steps", describing the concept, not the
>   API). A path on a `DiscreteTimeSequence` sampled `fs` times per unit time
>   divides by `fs`, so the answer is always a time and never an index.
> - **The direction is taken from where the path is at `start_time`**, exactly
>   as in Tier B. A consequence worth knowing, and tested: asking a *count* for
>   a level it has already passed reports `inf`, because it is then waiting for
>   a fall back to that level, which a count never does.

**The one real bug this surfaced, recorded so it is not reintroduced**
> `hitting_time` dispatched a whole process with
> `isinstance(process, RandomProcess)`. Most Tier A processes are **plain
> `RV`s** — `MarkovChain`, `ContinuousTimeMarkovChain`, `RandomWalk`, `MA`,
> `GG1`, `GGs` — so that check silently fell through and treated the *process*
> as a sample path. Broadened to `isinstance(process, RV)`, which is strictly
> wider (`RandomProcess` is an `RV`) and changes nothing for Tier B.

**Deliberately excluded: `NonHomogeneousPoissonProcess` and `CoxProcess`**
> Their counts do move in jumps, but they know their jumps on the *expected
> count* scale, not on the clock: the path stores rate-1 interarrivals and a
> `cumulative_rate`, and there is no stored clock time for the k-th jump.
> Getting one means inverting `Λ` — which the NHPP decision above explicitly
> rejected ("rejected as unnecessary... worth revisiting only if arrival times
> are exposed as a user-facing sequence"). Rather than smuggle a root-find in
> here, both raise their **own** `NotImplementedError` explaining precisely
> that, and pointing at what does work (`path(t)`, or `N[t].sim(...)` for the
> distribution of the count). This is the one place where "it jumps, therefore
> Tier A" fails, so it is worth the separate message rather than the generic
> one.

**Rationale**
> The roadmap's own recommendation was to build the hitting-time utility as one
> shared utility with per-process-type dispatch rather than three features, and
> the dispatch turned out to cost about fifteen lines: the hard part of Tier B
> is the reflection-principle machinery, and Tier A needs none of it. Keeping
> one entry point also means `start_time`, `max_time`, the RV-vs-path return
> convention, and the `inf`-means-never convention are defined once, which is
> what the follow-on `upcrossings` row will build on for *all* tiers at once.

**Alternatives Considered**
> **A separate `hitting_time_discrete` / `first_passage_time`** — rejected, per
> the roadmap row: a student should not have to know which tier their process
> is in to know which function to call, and "ruin time" is one use of this
> mechanism, not a separate build (the actuarial row it supersedes).
> **A `max_steps` argument alongside `max_time`** — rejected; for a
> discrete-time process a step is the time unit, so it would be two names for
> one thing, against the precedent set in "One Name for a Process's Starting
> Condition".
> **Raising when `step`/`tol` are passed with a Tier A process** — rejected as
> hostile: the arguments are harmless there, and `hitting_time(process, ...)`
> is meant to be callable uniformly across a class's worth of processes. They
> are documented as ignored and there are tests asserting they change nothing.
> **Bisecting on the count function** for the NHPP/Cox case, since a count is
> monotone — rejected for now: it would make the reported time approximate
> (to `tol`) in the one family where every other jump process is exact, and it
> is really the `Λ`-inversion question wearing a disguise.

**Accepted limitations, documented not fixed:**
> - A process that jumps far more often than the stretch being searched would
