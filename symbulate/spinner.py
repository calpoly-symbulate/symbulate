...
def _discrete_proportional(dist):
    ...
    slices = [{"label": str(v), "prob": p} for v, p in zip(values, raw) if p > 1e-7]
    if tail > 0.004:
...
def _build_slices(dist, mode):
    if getattr(dist, "discrete", False):
        return (
            _discrete_proportional(dist)
            if mode == "proportional"
            else _discrete_equal(dist, _DISC_EQUAL_N)
        )
    else:
        return (
            _continuous_proportional(dist, _CONT_BINS)
            if mode == "proportional"
...
def _draw_spinner(slices, dist, mode):
    ...
    # Outer border
    ax.add_patch(
        plt.Circle((0, 0), 1.0, fill=False, edgecolor="#2c3e50", linewidth=2.5)
    )

    # Center hub
    ax.add_patch(
        plt.Circle(
            (0, 0), 0.06, facecolor="#2c3e50", edgecolor="white", linewidth=2, zorder=10
        )
    )
